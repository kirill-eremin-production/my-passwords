...
export class SecureStorage {
     ...
     */
    private async encryptWithDeviceKey(
        data: string,
        deviceKey: string
    ): Promise<string> {
        try {
     ...
     */
    private async decryptWithDeviceKey(
    ...
    private async generateDeviceKey(): Promise<string> {
        ...
        try {
            ...
            console.log(
                '✅ DeviceKey сгенерирован успешно:',
                deviceKey.substring(0, 20) + '...'
            )
            return deviceKey
        } catch (error) {
            console.error(
                '❌ Ошибка генерации device key, используем fallback:',
                error
            )
    ...
    private async getRawData(key: string): Promise<string | null> {
        try {
            ...
            const item = await new Promise<SecureStorageItem | null>(
                (resolve, reject) => {
                    const request = store.get(key)
                    request.onsuccess = () => resolve(request.result || null)
                    request.onerror = () =>
                        reject(new Error('Ошибка чтения из IndexedDB'))
                }
            )
    ...
    async storeEncryptedData(key: string, data: string): Promise<void> {
        try {
            console.log(
                '🔄 SecureStorage.storeEncryptedData: начинаем сохранение для ключа:',
                key
            )
            ...
            const doubleEncrypted = await this.encryptWithDeviceKey(
                data,
                deviceKey
            )
            ...
            console.log(
                '🎉 SecureStorage.storeEncryptedData: сохранение завершено'
            )
        } catch (error) {
            console.error(
                '❌ SecureStorage.storeEncryptedData: ошибка сохранения:',
                error
            )
            throw error
        }
    ...
    async removeData(key: string): Promise<void> {
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                request.onsuccess = () => resolve()
                request.onerror = () =>
                    reject(new Error('Ошибка удаления из IndexedDB'))
            })
        } catch (error) {
    ...
    async cleanupOldData(
        ...
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                request.onerror = () =>
                    reject(new Error('Ошибка очистки старых данных'))
            })
        } catch (error) {
