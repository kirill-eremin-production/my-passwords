...
export function validateMasterPassword(password: string): PasswordValidationResult {
    ...
    if (!hasSpecialChars) errors.push('Пароль должен содержать специальные символы');
 ...
 */
export function checkPasswordSecurity(password: string): { isSecure: boolean; warnings: string[] } {
...
export function decryptMasterPasswordLocally(encryptedData: string, credentialId: string): string {
    try {
        ...
        const keyPrefix = version === '2.0' ? 'master-key-v2-2024' : 'master-key-2024';
        const hmacPrefix = version === '2.0' ? 'hmac-key-v2-2024' : 'hmac-key-2024';
        ...
        if (expectedHmac !== combined.hmac) {
            throw new Error('Неверный HMAC - данные могли быть изменены или повреждены');
        }
