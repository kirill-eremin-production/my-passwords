...
export async function registerBiometric(req: Request, res: Response) {
    ...
    if (!session) {
        console.log(
            '🔄 Создание новой валидной сессии для регистрации биометрии'
        )
        const {
            createNewSession,
        } = require('../../middlewares/authorization/utils')
    ...
    try {
        ...
        console.log(
            '🔍 Регистрация - challenge из clientData:',
            challengeFromClient
        )
        ...
        const verificationResult =
            await verifyRegistrationResponse(verification)

        if (!verificationResult.verified) {
            console.error(
                '❌ Верификация регистрации не прошла:',
                verificationResult
            )
            res.status(400).json({
                success: false,
        ...
        console.log(
            '🔄 Сохраняем credential с правильным COSE публичным ключом...'
        )
