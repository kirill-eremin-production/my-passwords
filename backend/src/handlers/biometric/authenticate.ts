...
export async function authenticateWithBiometric(req: Request, res: Response) {
    ...
    try {
        ...
        if (!credential) {
            res.status(404).json({
                error: 'Биометрические учетные данные не найдены',
            })
            return
        }
        ...
        if (!challengeData) {
            ...
            res.status(400).json({
                error: 'Отсутствует challenge для данной сессии',
            })
            return
        }
        ...
        console.log(
            '🔍 Валидируем clientData с challenge:',
        ...
        if (!clientDataValidation.isValid) {
            console.log(
                '❌ Валидация clientData неуспешна:',
                clientDataValidation.error
            )
        ...
        const newCounter =
            extractCounterFromAuthenticatorData(authenticatorData)
        ...
        if (newCounter === -1) {
            ...
            res.status(400).json({
                error: 'Ошибка извлечения counter из authenticatorData',
            })
            return
        }
        ...
        try {
            ...
            res.status(500).json({
                error: 'Ошибка верификации подписи WebAuthn',
            })
            return
        }
