...
export function readBiometricStore(): string {
    try {
        ...
        console.error(
            'Ошибка чтения зашифрованного биометрического хранилища:',
            error
        )
        return '{}'
    }
...
export function writeBiometricStore(data: string) {
    try {
        ...
        console.error(
            'Ошибка записи в зашифрованное биометрическое хранилище:',
            error
        )
        throw error
    }
 ...
 */
export function getBiometricCredentialById(
    credentialId: string
 ...
 */
export function getBiometricCredentialsBySession(
    sessionId: string
): BiometricCredential[] {
    const store: Record<string, BiometricCredential> = JSON.parse(
        readBiometricStore() || '{}'
    )
    return Object.values(store).filter(
        (cred: BiometricCredential) => cred.sessionId === sessionId
    )
}
 ...
 */
export function updateBiometricCredentialCounter(
    credentialId: string,
    newCounter: number
): boolean {
    const store = JSON.parse(readBiometricStore() || '{}')
...
export function readChallengesStore(): string {
    try {
        ...
        console.error(
            'Ошибка чтения зашифрованного хранилища challenges:',
            error
        )
        return '{}'
    }
...
export function writeChallengesStore(data: string) {
    try {
        ...
        console.error(
            'Ошибка записи в зашифрованное хранилище challenges:',
            error
        )
        throw error
    }
 ...
 */
export function getChallengeBySessionId(
    sessionId: string
