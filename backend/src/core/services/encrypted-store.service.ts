...
export class EncryptedStoreService {
  ...
  constructor() {
    ...
    if (this.encryptionKey === 'default-key-change-me-in-production') {
      if (process.env.NODE_ENV === 'production') {
        throw new Error(
          '🚨 КРИТИЧЕСКАЯ ОШИБКА: Стандартный ключ шифрования в production! Установите FILE_ENCRYPTION_KEY'
        );
      } else {
        console.warn(
          '⚠️ ПРЕДУПРЕЖДЕНИЕ: Используется стандартный ключ шифрования! Установите FILE_ENCRYPTION_KEY в .env'
        );
      }
    }
  }
  ...
  readEncrypted(filePath: string): string {
    try {
      ...
      try {
        ...
        console.warn(
          '⚠️ Файл не в зашифрованном формате, создаем новый зашифрованный файл с пустыми данными'
        );
        ...
        console.log(
          `📝 Создана резервная копия старого файла: ${backupPath}`
        );
  ...
  private decryptByVersion(encryptedData: EncryptedData): string {
    ...
    const iterations =
      ...
      (version === '1.0' ? 10000 : this.CURRENT_ITERATIONS);
    ...
    if (version !== this.CURRENT_VERSION) {
      console.log(
        `📄 Расшифровка файла версии ${version} (текущая: ${this.CURRENT_VERSION})`
      );
    }
  ...
  migrateToEncrypted(filePath: string): void {
    try {
      ...
      if (currentVersion && currentVersion !== this.CURRENT_VERSION) {
        console.log(
          `🔄 Обновление файла ${filePath} с версии ${currentVersion} до ${this.CURRENT_VERSION}...`
        );
        ...
        console.log(
          `🔄 Миграция файла ${filePath} в зашифрованный формат...`
        );
  ...
  validateEncryptionKey(): boolean {
    if (!this.encryptionKey || this.encryptionKey.length < 16) {
      if (process.env.NODE_ENV !== 'production') {
        console.error(
          '❌ Ключ шифрования слишком короткий (минимум 16 символов)'
        );
      }
      return false;
    }
